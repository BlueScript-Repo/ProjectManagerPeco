    ...
    protected void generateBOQ(String projectId, String boqName, String[] product, String[] moc,
       ...
       for(int i=0; i<dNo.length; i++)
        ...
        if (Boolean.valueOf(isOffer) && (venderName != null && !(venderName.isEmpty()))) {
            ...
            notificationUtil.pushNotification(userName, vendorDetails.getContactEmail(), "PECO Projects : Inventory Inquiry", "Greeting of the day from PECO Projects. Please find attached the Inventory requirment. Please update the same and respond.", file.getName(), "INBOX", new SimpleDateFormat("dd/mm/yyyy").format(new Date()));

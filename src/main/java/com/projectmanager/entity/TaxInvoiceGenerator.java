...
public class TaxInvoiceGenerator {
	...
	public void generateAndSendTaxInvoice(TaxInvoiceDetails taxInvoiceDetails, String sender, String userName) {
		...
		notificationUtil.pushNotification(userName,taxInvoiceDetails.getEmailAddress(),"Tax Invoice : PECO Industries", "Please find attached the Tax Invoice.",fileToAttach.replace("/", "_") + ".pdf" +";"+fileToAttach.replace("/", "_") + "_Annexure.xls","INBOX", new SimpleDateFormat("dd/mm/yyyy").format(new Date()));
